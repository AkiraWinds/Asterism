...
def search_web(api_key: str, query: str, count: int = 3) -> list[dict]:
    ...
    response (auth failure, rate limit, etc.) — callers (the resolution chain)
    treat that the same as "web search unavailable" and fall through to the
    next step, never surfacing a raw HTTP error to the end user."""
    response = httpx.get(
