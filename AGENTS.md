...
### Verify Third-Party API Assumptions Against the Installed Version, Not Memory (2026-08-21)
`react-resizable-panels` v4 renamed its exports (`PanelGroup`/`PanelResizeHandle` → `Group`/`Separator`) and changed how `minSize`/`defaultSize` are interpreted (bare numbers are now pixels, not percentages — a string with a unit is required for percentages). A plan written from training-data knowledge of the library's older API got both wrong; per-task review caught the renamed exports and a missing persistence hook, but the pixel/percentage bug survived per-task review and was only caught by the final whole-branch review, which read the installed package's actual `.d.ts`/compiled dist instead of trusting the plan's code samples. **Apply this to any new dependency**: after `npm install`, check the installed version's actual type declarations/dist for API surface and unit semantics before writing code against it — don't assume training-data familiarity with a library still matches its current major version.

### Tailwind Class Names Must Be Literal Strings, Never Interpolated (2026-08-21)
Tailwind's build-time scanner only recognizes class names that appear as literal tokens in source files — `` `${SOME_VAR}:inline` `` is invisible to it even though it renders the right string at runtime, so the corresponding utility class is silently never generated and the intended style never applies. This shipped past per-task review in the resizable-panels branch (a `DATE_REVEAL_BREAKPOINT` constant interpolated into a template literal) and was only caught by the final review, which actually compiled the CSS and grepped for the expected breakpoint rather than reading the JSX. **When reviewing or writing Tailwind classes driven by a variable, always inline the literal class string** (a comment can still explain the breakpoint choice), and when verifying container-query or other dynamically-referenced utilities, compile the CSS and check the output — reading the source is not sufficient proof the class exists.
