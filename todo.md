...
- **Wiki compile — a crash between writing aspect files and writing the overview leaves permanently orphaned `.md` files**: `run_compile` writes each aspect file, then the overview page, as two separate sequential steps (`backend/app/wiki/compile.py`); if the process dies (crash, OOM-kill, forced restart) after some/all aspect files are written but before the overview's `write_text()` lands, those aspect `.md` files are never referenced in any overview's `aspects` frontmatter list — so no future compile run's stale-aspect-deletion logic (keyed on the overview's own recorded list) will ever find or clean them up. Worse, `aspect_slug()`'s numeric-suffix collision resolution treats those orphans as permanently taken, so every future aspect proposal for that concept that would slugify to the same base gets pushed onto `-2`/`-3` suffixes forever. **Why:** found in the 2026-08-01 wiki-compile final-review fix pass — same failure class CLAUDE.md's "File-Existence-Inferred Status" lesson already flags for the `meta.json`/`content.md` triad, now showing up in the wiki-page writer too. Revisit by writing the overview page first with the full aspect slug list already decided, or by treating the aspect+overview write sequence as a mini-transaction with a rollback/cleanup step on failure.
- **Wiki compile — no cleanup path for concepts that stop qualifying or are deleted from `graph.db`**: once a concept's page (overview, and now aspect pages too) is written, nothing ever removes it if the concept later drops below `MIN_PROVENANCE_COUNT` or is deleted from `graph.db` entirely — `run_compile` only ever adds/updates pages for `qualifying` concepts, it never diffs "what's on disk" against "what still qualifies" to find pages that should be retired. **Why:** this isn't a regression introduced by the aspect-split work — overview pages already had this gap since Phase 6d — but splitting each concept into an overview plus N aspect files multiplies the number of orphan-prone files left behind per stale concept. Revisit alongside the crash-orphan item above, since both need the same underlying capability: a reconciliation pass that knows the full current set of "pages this concept should own" and can delete anything outside it.
