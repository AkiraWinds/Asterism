...
- **Golden concepts (watchlist) will surface as wiki orphans**: `wiki/selection.py` lets `golden=True` concepts bypass the `MIN_PROVENANCE_COUNT` threshold (2026-08-01 entity-extraction/watchlist plan, decision to make approved watchlist concepts visible in the wiki even with zero source links). A freshly approved golden concept typically has no edges yet, so `compile.py`'s `find_orphan_concepts` will reliably flag it in index.md's "needs attention" list. This is expected noise from that decision, not a bug — revisit if it becomes annoying (e.g. suppress golden concepts from the orphan list specifically, vs. concepts that should have provenance but don't).
- **Watchlist resolver threshold compares term-embeddings to definition-embeddings**: `watchlist/resolver.py`'s `_MATCH_THRESHOLD = 0.85` compares an embedding of the watchlist *term* against stored concept embeddings, which are built from concept *definitions* (and, after the 2026-08-01 grounding-desync fix, specifically from grounded definitions when applicable). These are two different kinds of text, so the 0.85 cutoff wasn't empirically calibrated against this specific embedding distribution. Revisit if the resolver starts producing surprising graph-match/no-match calls in practice.
- **`ConceptNode.golden` has no frontend consumer yet**: the API now correctly returns `golden` on concepts (fixed 2026-08-01 — it was in the schema since Task 1 but never populated at construction sites), but nothing in `frontend/` reads it yet. Needs a UI affordance (e.g. a badge) before the golden flag is visible to users anywhere outside direct API calls.
